...
public final class ToModel
{
	//使用这个只能全使用数据库表的值传；不能赋值
	public static <ToModel> ToModel DataRowToModel(DataRow dr)
	{
		java.lang.Class type = ToModel.class;
		ToModel md = (ToModel)type.newInstance(); //泛型 是咧化对象
		for (var prop : type.GetProperties()) // 遍历所以属性
		{
			prop.SetValue(md, dr.get(prop.Name), null);
		}
		return md;
	}
}
